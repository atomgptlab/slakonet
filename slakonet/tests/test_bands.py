...
def get_atoms(jid=''):
    for i in dft_3d:
        if i['jid']==jid:
            return Atoms.from_dict(i['atoms']),i['optb88vdw_bandgap'],i['mbj_bandgap']
...
def get_gap(jid='JVASP-1002',model=None,plot=False,default_points=2,line_density=20):
    #jid='JVASP-14636'
    atoms,opt_gap,mbj_gap=get_atoms(jid) #Atoms.from_dict(get_jid_data(jid=jid,dataset='dft_3d')['atoms'])
    ...
    if plot:
        ...
        efermi=properties['efermi'].squeeze().detach().numpy().tolist()
        for i in range(eigenvalues.shape[-1]):  # Plot each band
         plt.plot(eigenvalues[0, :, i].real.detach().numpy() * H2E-efermi,c='blue')
        ...
        gap=jid+'_'+atoms.composition.reduced_formula+' Bandgap: '+str(round(bandgap,2))
        #plt.title(gap)
...
model_path="/mnt/c/Users/kchoudh2/'OneDrive - Johns Hopkins'/SlakoNet/slakonet_v1"
