...
def fermi_search(
    ...
    def middle_gap_approximation(scale_factor):
        ...
        scale_flat = (
            scale_factor if k_weights is None else scale_factor.reshape(shp)
        )
    ...
    if isinstance(e_mask, Basis):
        ...
        if k_weights is not None:
            e_mask = e_mask.repeat(1, k_weights.shape[-1], 1).reshape(
                e_vals.shape
            )
    ...
    if tol is None:  # auto-assign if no tolerance was given
        tol = {torch.float64: 1e-10, torch.float32: 1e-5, torch.float16: 1e-2}[
            dtype
        ]
