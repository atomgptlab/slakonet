...
from slakonet.utils import split_by_size
...
class Basis:
    ...
    def __init__(
        self,
        atomic_numbers: Union[List[Tensor], Tensor],
        ...
        if not all(
            [
                isinstance(j, int) and isinstance(i, list)
                for i in shell_dict.values()
                for j in i
            ]
        ):
        ...
        self.shell_ns, self.shell_ls = torch.tensor(
            [
                (i, l)
                for n in self.atomic_numbers.view(-1)
                if n != 0
                for i, l in enumerate(shell_dict[int(n)])
            ],
            **kwargs,
        ).T
    ...
    def device(self, *args):
        ...
        raise AttributeError(
            "Basis object's dtype can only be modified "
            'via the ".to" method.'
        )
    ...
    def orbs_per_atom(self) -> Tensor:
        ...
        return self._orbitals_per_species[
            self.atomic_numbers.view(-1)
    ...
    def orbs_per_res(self):
        ...
        return (
            self.orbs_per_shell if self.shell_resolved else self.orbs_per_atom
        )
    ...
    def on_atoms(self) -> Tensor:
        ...
        if (opa := self.orbs_per_atom).ndim == 1:
            ...
            return pack(
                [_repeat_range(o, device=self.__device) for o in opa], value=-1
            )
    ...
    def on_shells(self) -> Tensor:
        ...
        if (ops := self.orbs_per_shell).ndim == 1:
            ...
            return pack(
                [_repeat_range(o, device=self.__device) for o in ops], value=-1
            )
    ...
    def shells_per_atom(self) -> Tensor:
        ...
        return self._shells_per_species[self.atomic_numbers.view(-1)].view_as(
            self.atomic_numbers
        )
    ...
    def mask(self, geometry, cutoff):
        ...
        mask_l = (
            pack(
                [
                    dist.repeat_interleave(n_orb, 0).repeat_interleave(
                        n_orb, 1
                    )
                    for dist, n_orb in zip(mask_d, self.orbs_per_atom)
                ]
            )
            if geometry._n_batch is not None
            else mask_d.repeat_interleave(
                self.orbs_per_atom, 0
            ).repeat_interleave(self.orbs_per_atom, 1)
        )
        ...
        mask_s = (
            pack(
                [
                    ...
                    for dist, n_orb in zip(mask_d, self.shells_per_atom)
                ]
            )
            if geometry._n_batch is not None
            else mask_d.repeat_interleave(
                self.shells_per_atom, 0
            ).repeat_interleave(self.shells_per_atom, 1)
        )
    ...
    def matrix_shape(self, form: Form) -> Size:
        ...
        try:
            shape = {
                "full": self.orbital_matrix_shape,
                "shell": self.shell_matrix_shape,
                "atomic": self.atomic_matrix_shape,
            }[form]
            return shape

        except KeyError as e:  # Warn if an unknown "form" option is used
            raise ValueError(
                f'"{form}" is not a valid option for "form".'
                f"valid values are {Form.__args__}"
            )

    def azimuthal_matrix(
        self,
        form: Form = "full",
        sort: bool = False,
        mask_on_site: bool = False,
        ...
        if form == "full":
            ...
            if batch:
                basis_list = pack(
                    split_by_size(basis_list, self.n_orbitals), value=-1
                )
        else:
    ...
    def shell_number_matrix(self, form: Form = "full") -> Tensor:
        ...
        if form == "full":  # If the full matrix is required: expand the s_mat
            s_mat = s_mat.repeat_interleave(
                torch.where(
                    self.shell_ls == -1, 0, self.shell_ls * 2 + 1
                ).view(-1)
            )
            if self.atomic_numbers.dim() == 2:  # "if batch"
                s_mat = pack(
                    torch.split(s_mat, tuple(self.n_orbitals)), value=-1
                )
        elif form == "atomic":
            ...
            if self.atomic_numbers.dim() == 2:  # "if batch"
                s_mat = pack(
                    torch.split(s_mat.view(-1), tuple(self.n_atoms)), value=-1
                )

        s_mat = (
            torch.from_numpy(s_mat) if isinstance(s_mat, np.ndarray) else s_mat
        )
    ...
    def index_matrix(self, form: Form = "full") -> Tensor:
        ...
        if form == "full":
            ...
            i_mat = (
                arange(self.n_atoms.max(), device=self.__device)
                .expand_as(ans)
                .repeat_interleave(self._shells_per_species[ans.view(-1)])
            )
...
def _rows_to_NxNx2(
    vec: Tensor, shape: Size, pad: Optional[Any] = None
) -> Tensor:
