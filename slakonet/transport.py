...
from typing import Tuple, Optional, Union, Dict  # Add Dict here
...
class SlakoNetTransportCalculator:
    ...
    def _setup_hamiltonians(
        self, scat_atoms, lead_atoms, scat_kpts, lead_kpts
    ):
    ...
    def _align_fermi_levels(self):
        ...
        if E_diff > 0.1:
            ...
            print(
                f"      This may indicate incompatible electronic structures."
            )
    ...
    def calculate_transmission(
        ...
        if verbose:
            ...
            print(
                f"  Energy range: {energies[0]:.2f} to {energies[-1]:.2f} eV"
            )
        ...
        if verbose:
            ...
            print(
                f"  T_max:   {np.max(T_e):.4f} at E = {energies[np.argmax(T_e)]:.2f} eV"
            )
    ...
    def calculate_conductance(
        ...
        """
        Calculate conductance spectrum from transmission.
        ...
        G_G0 : np.ndarray
            Conductance in units of G₀ = 2e²/h
        """
        G0 = 2 * e**2 / h  # Quantum conductance (S)
        G0_uS = G0 * 1e6  # In microsiemens = 77.48 μS

        G_uS = G0_uS * T_e  # Conductance in μS
        G_G0 = T_e  # Conductance in G₀ units

        return G_uS, G_G0

    def get_zero_bias_conductance(
        self,
        energies: np.ndarray,
        T_e: np.ndarray,
    ) -> Dict[str, float]:
        """
        Extract zero-bias conductance G(E_F).

        Returns:
        --------
        dict with keys:
            'G_uS': float - Conductance in microsiemens
            'G_G0': float - Conductance in G₀ units
            'G_S': float - Conductance in Siemens
        """
        idx_EF = np.argmin(np.abs(energies))
        T_EF = T_e[idx_EF]
    ...
    def calculate_iv_curve(
        ...
        if verbose:
            ...
            print(
                f"  Voltage range: {voltages[0]:.2f} to {voltages[-1]:.2f} V"
            )
    ...
    def plot_all(
        ...
        if voltages is not None and currents is not None:
            ...
            ax1.set_title(
                "Transmission Spectrum", fontsize=12, fontweight="bold"
            )
            ...
            if self.gap_scat > 0.01:
                ...
                ax1.axvspan(
                    ...
                    alpha=0.1,
                    color="gray",
                    label=f"Gap ({self.gap_scat:.2f} eV)",
                )
            ...
            ax3.set_title(
                "Differential Conductance", fontsize=12, fontweight="bold"
            )
            ...
            ax1.set_title(
                "Transmission Spectrum", fontsize=14, fontweight="bold"
            )
            ax1.grid(True, alpha=0.3, linestyle=":")
...
def silicon_nanowire_example():
    ...
    energies = np.arange(-3, 3, 0.02)
    T_e = calc.calculate_transmission(energies)
    G_uS, G_G0 = calc.calculate_conductance(energies, T_e)
    G_zero_bias = calc.get_zero_bias_conductance(energies, T_e)
    ...
    print(
        f"  Threshold V:     ~{voltages[np.where(np.abs(currents) > 1e-7)[0][0]]:.2f} V"
    )
    print(f"  I @ ±2V:         ±{abs(currents[-1])*1e6:.2f} μA")
    print(f"\n  Conductance at E_F:")
    print(f"    G(E_F):        {G_zero_bias['G_uS']:.3f} μS")
    print(f"    G(E_F):        {G_zero_bias['G_G0']:.4f} G₀")
    print(f"    G_max:         {np.max(G_uS):.3f} μS")
    print("=" * 70 + "\n")
...
def aluminum_nanowire_example():
    ...
    print(f"  I @ ±2V:         ±{abs(currents[-1])*1e6:.2f} μA")
    print(
        f"  I-V behavior:    {'Linear (Ohmic)' if np.std(np.gradient(currents)) < 1e-8 else 'Nonlinear'}"
    )
    print(f"\n  Conductance at E_F:")
    print(f"    G(E_F):        {G_zero_bias['G_uS']:.3f} μS")
    print(f"    G(E_F):        {G_zero_bias['G_G0']:.4f} G₀")
...
def compare_si_al():
    ...
    axes[0, 1].plot(
        si_V, si_I * 1e6, "b-", linewidth=2, marker="o", markersize=3
    )
    ...
    axes[0, 2].plot(
        si_V, si_G * 1e6, "b-", linewidth=2, marker="o", markersize=3
    )
    ...
    axes[1, 1].plot(
        al_V, al_I * 1e6, "r-", linewidth=2, marker="o", markersize=3
    )
    ...
    axes[1, 2].plot(
        al_V, al_G * 1e6, "r-", linewidth=2, marker="o", markersize=3
    )
    ...
    print("-" * 70)
    print(
        f"{'Band gap (eV)':<30} {si_calc.gap_scat:<20.3f} {al_calc.gap_scat:<20.3f}"
    )
    print(
        f"{'T(E_F)':<30} {si_T[np.argmin(np.abs(si_E))]:<20.4f} {al_T[np.argmin(np.abs(al_E))]:<20.4f}"
    )
    print(f"{'Max T(E)':<30} {np.max(si_T):<20.4f} {np.max(al_T):<20.4f}")
    print(f"{'I @ 2V (μA)':<30} {si_I[-1]*1e6:<20.3f} {al_I[-1]*1e6:<20.3f}")
    print(
        f"{'G @ V=0 (μS)':<30} {si_G[len(si_V)//2]*1e6:<20.3f} {al_G[len(al_V)//2]*1e6:<20.3f}"
    )
